mutable struct spec_lchirp_transfo <: discrete
  dim::Int64
  pt::Array{Float64,1}
  pω::Array{Float64,1}
...
"""
function setSpecTKernel(pt::Array{Float64,1},pω::Array{Float64,1},Dpt::Float64,Dpω::Float64, σ::Float64, angle_min::Float64, angle_max::Float64)
  ...
  return spec_lchirp_transfo(dim, pt, pω, p, Npt, Npω, Dpt, Dpω, nb_points_param_grid, g, meshgrid, σ, bounds)
end

mutable struct operator_spec_lchirpT <: operator
  ker::DataType
  dim::Int64
  sigma::Float64
  bounds::Array{Array{Float64,1},1}
...
function setSpecTOperator(kernel::spec_lchirp_transfo,a0::Array{Float64,1},x0::Array{Array{Float64,1},1},w::Array{Float64,1})
...
v[l] = -N * (1 + c ^ 2) * exp(-2 * pi * σ ^ 2 * (c * N * t + η * N - ω) ^ 2 / (1 + σ ^ 4 * c ^ 2 * N ^ 2)) * (N ^ 3 * c ^ 3 * σ ^ 6 - 4 * t * N ^ 2 * pi * σ ^ 4 * (η * N - ω) * c ^ 2 + 4 * N * (-N ^ 2 * pi * η ^ 2 * σ ^ 4 + 2 * N * pi * η * ω * σ ^ 4 - pi * ω ^ 2 * σ ^ 4 + pi * t ^ 2 + σ ^ 2 / 4) * c + 4 * t * pi * (η * N - ω)) * σ ^ 3 * (1 + σ ^ 4 * c ^ 2 * N ^ 2) ^ (-5//2)
  ...
  operator_spec_lchirpT(typeof(kernel),kernel.dim,kernel.σ,kernel.bounds,normObs,phiVect,d1phiVect,d11phiVect,d2phiVect,y,c,d10c,d01c,d11c,d20c,d02c,ob,d1ob,d11ob,d2ob,correl,d1correl,d2correl);
end

function computePhiu(u::Array{Float64,1},op::blasso.operator_spec_lchirpT)
...
function minCorrelOnGrid(Phiu::Array{Array{Float64,1},1},kernel::blasso.spec_lchirp_transfo,op::blasso.operator,positivity::Bool=true)
...
"""
function setbounds(op::blasso.operator_spec_lchirpT,positivity::Bool=true,ampbounds::Bool=true)
  x_low=op.bounds[1];
  x_up=op.bounds[2];

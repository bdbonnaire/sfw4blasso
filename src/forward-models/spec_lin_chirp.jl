...
""" setspecKernel(px, py, Dpx, Dpy, sigma, bounds, angle_min, angle_max)
...
"""
function setspecKernel(px::Array{Float64,1},py::Array{Float64,1},Dpx::Float64,Dpy::Float64,
  sigma::Float64, angle_min::Float64, angle_max::Float64)
  ...
  nb_grid_samples = [ abs(η_min - η_max)*freq_coeff, Npx* abs(θ_min - θ_max) * angle_coeff]
  nb_grid_samples = convert(Int64, nbpointsgrid)
  # buiding the grid
  ...
  return spec_lchirp(dim, px, py, p, Npx, Npy, Dpx, Dpy, nb_grid_samples, g, sigma, bounds)
end

mutable struct op_spec_lchirp <: operator
  ker::DataType
  dim::Int64
  sigma::Float64
  bounds::Array{Array{Float64,1},1}
